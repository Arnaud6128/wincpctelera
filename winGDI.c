...
extern const SCPCPalette gCpcPalette[];
...
#define NB_RECT 10
static RECT _rectHelper[NB_RECT];
static DWORD _rectColor[NB_RECT];
void DrawRectHelper(int rectId, int x, int y, int cx, int cy, int color)
{
	if (rectId < NB_RECT)
	{
		_rectColor[rectId] = color;

		int pixelW = 1 * COEF;
		if (gAmstrad._curVideoConf._videoMode == 0)
			pixelW = 4 * COEF;
		else if (gAmstrad._curVideoConf._videoMode == 0)
			pixelW = 2 * COEF;

		SetRect(&_rectHelper[rectId], BORDER_CX + x * pixelW, BORDER_UP_CY + y * COEF, BORDER_CX + (x + cx) * pixelW, BORDER_UP_CY + (y + cy) * COEF);
	}
}

void CleanRectHelper()
{
	memset(_rectHelper, 0, sizeof(_rectHelper));
}
...
static void wincpct_redraw()
{
	...
	{
		RECT rect = _rectHelper[i];
		DWORD color = _rectColor[i];

		HBRUSH brush = CreateSolidBrush(gCpcPalette[color].rgb);
		FrameRect(_memDC, &rect, brush);
...
static void win_cpct_computeWindowsSize(RECT* pWindowRect)
{
	...
	SetRect(pWindowRect, 0, posY, size.cx, size.cy);
}
